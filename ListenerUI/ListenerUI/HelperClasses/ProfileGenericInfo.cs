...
namespace MeterReader.TestHelperClasses
{
    public static class ProfileGenericInfo
    {
        ...
        public static void FillTables()
        {
            ...
            int inUseEntries = 0;
            DateTime meterRTC = DateTime.Now;
                ...
                foreach (string selectedProfile in profileList)
                {
                    ...
                    switch (gotProfile)
                    {
                        ...
                        case "1.0.99.1.0.255":
                            ...
                            meterRTC = DateTime.ParseExact(parse.GetProfileValueString(SetGetFromMeter.GetDataFromObject(ref DLMSObj, 8, "0.0.1.0.0.255", 2).Trim()).Trim(), "dd/MM/yyyy hh:mm:ss tt", CultureInfo.InvariantCulture);
                            int currentLSIP = int.Parse(parse.GetProfileValueString(SetGetFromMeter.GetDataFromObject(ref DLMSObj, 7, "1.0.99.1.0.255", 4).Trim())) / 60;
                            if (inUseEntries >= 1)
                            {
                                DateTime endEntry = new DateTime(meterRTC.Year, meterRTC.Month, meterRTC.Day, meterRTC.Hour, meterRTC.Minute, 0);
                                DateTime startEntry = endEntry.AddMinutes(-currentLSIP);
                                startDT = startEntry.ToString("dd/MM/yyyy HH:mm");
                                endDT = endEntry.ToString("dd/MM/yyyy HH:mm");
                            }
                            else
                            {
                                DateTime DateTimeToSet = meterRTC;   //.AddMinutes(currentLSIP);
                                NextIP(ref meterRTC, currentLSIP, 59, 1);
                                DLMSObj.SetValue(8, "0.0.1.0.0.255", 2, SetGetFromMeter.GetRTCSetString(DateTimeToSet), MeterIdentity.deviationByte);
                                SetGetFromMeter.Wait(2 * 1000);
                                meterRTC = DateTime.ParseExact(parse.GetProfileValueString(SetGetFromMeter.GetDataFromObject(ref DLMSObj, 8, "0.0.1.0.0.255", 2).Trim()).Trim(), "dd/MM/yyyy hh:mm:ss tt", CultureInfo.InvariantCulture);
                                DateTime endEntry = new DateTime(meterRTC.Year, meterRTC.Month, meterRTC.Day, meterRTC.Hour, meterRTC.Minute, 0);
                                DateTime startEntry = endEntry.AddMinutes(-currentLSIP);
                                startDT = startEntry.ToString("dd/MM/yyyy HH:mm");
                        ...
                        case "1.0.99.2.0.255":
                            ...
                            if (inUseEntries >= 1)
                            {
                                DateTime startEntry = new DateTime(meterRTC.Year, meterRTC.Month, meterRTC.Day, 00, 00, 00);
                                DateTime endEntry = new DateTime(meterRTC.Year, meterRTC.Month, meterRTC.Day, 23, 00, 00);
                                startDT = startEntry.ToString("dd/MM/yyyy HH:mm");
                                endDT = endEntry.ToString("dd/MM/yyyy HH:mm");
                            }
                            else
                            {
                                DateTime DateTimeToSet = meterRTC.AddDays(1);
                                DLMSObj.SetValue(8, "0.0.1.0.0.255", 2, SetGetFromMeter.GetRTCSetString(DateTimeToSet), MeterIdentity.deviationByte);
                                SetGetFromMeter.Wait(2 * 1000);
                                meterRTC = DateTime.ParseExact(parse.GetProfileValueString(SetGetFromMeter.GetDataFromObject(ref DLMSObj, 8, "0.0.1.0.0.255", 2).Trim()).Trim(), "dd/MM/yyyy hh:mm:ss tt", CultureInfo.InvariantCulture);
                                DateTime startEntry = new DateTime(meterRTC.Year, meterRTC.Month, meterRTC.Day, 00, 00, 00);
                                DateTime endEntry = new DateTime(meterRTC.Year, meterRTC.Month, meterRTC.Day, 23, 00, 00);
                                startDT = $"{startEntry.ToString("dd/MM/yyyy ")}00:00";
                                endDT = $"{endEntry.ToString("dd/MM/yyyy ")}23:00";
        ...
        public static void FillTables(ref WrapperComm WrapperObj)
        {
            DLMSParser parse = new DLMSParser();
            int inUseEntries = 0;
            DateTime meterRTC = DateTime.Now;
            ...
            try
            {
                ...
                foreach (string selectedProfile in profileList)
                {
                    string profileLNNameData = "";
                    string gotProfile = "";
                    if (SetGetFromMeter.GetDataFromWrapperObject(ref WrapperObj, (ObjectType)7, $"{selectedProfile.Split('-')[1].Trim()}", 1, out profileLNNameData))
                    {
                        gotProfile = parse.GetProfileValueString(profileLNNameData);
                    }
                    if (gotProfile.Split('.').Length == 6)
                    {
                        availableProfileList.Add(selectedProfile);
                    ...
                    switch (gotProfile)
                    {
                        //Nameplate
                        case "0.0.94.91.10.255":
                            var nameplateData = WrapperObj.GetNameplateProfileDataTable();
                            nameplateDT = ((DataTable)nameplateData["DataTable"]).Copy();
                            IsNameplateAvailable = true;
                            break;
                        //Instantaneous Profile
                        case "1.0.94.91.0.255":
                            var instantData = WrapperObj.GetSingleEntryProfileDataTable($"{selectedProfile.Split('-')[1].Trim()}", $"{selectedProfile.Split('-')[2].Trim()}");
                            instantDT = ((DataTable)instantData["DataTable"]).Copy();
                            IsInstantAvailable = true;
                            break;
                        //Billing Profile
                        case "1.0.98.1.0.255":
                            var billingData = WrapperObj.GetSingleEntryProfileDataTable($"{selectedProfile.Split('-')[1].Trim()}", $"{selectedProfile.Split('-')[2].Trim()}");
                            billingDT = ((DataTable)billingData["DataTable"]).Copy();
                            IsBillAvailable = true;
                            break;
                        //Block Load Profile
                        case "1.0.99.1.0.255":
                            string startDT = string.Empty;
                            string endDT = string.Empty;
                            string inUseEntriesData = "", meterRTCData = "", currentLSIPData = "";
                            SetGetFromMeter.GetDataFromWrapperObject(ref WrapperObj, (ObjectType)7, "1.0.99.1.0.255", 7, out inUseEntriesData);
                            SetGetFromMeter.GetDataFromWrapperObject(ref WrapperObj, (ObjectType)8, "0.0.1.0.0.255", 2, out meterRTCData);
                            SetGetFromMeter.GetDataFromWrapperObject(ref WrapperObj, (ObjectType)7, "1.0.99.1.0.255", 4, out currentLSIPData);

                            inUseEntries = Convert.ToInt32(parse.GetProfileValueString(inUseEntriesData));
                            meterRTC = DateTime.ParseExact(parse.GetProfileValueString(meterRTCData.Trim()).Trim(), "dd/MM/yyyy hh:mm:ss tt", CultureInfo.InvariantCulture);
                            int currentLSIP = int.Parse(parse.GetProfileValueString(currentLSIPData.Trim())) / 60;
                            if (inUseEntries >= 1)
                            {
                                DateTime endEntry = new DateTime(meterRTC.Year, meterRTC.Month, meterRTC.Day, meterRTC.Hour, meterRTC.Minute, 0);
                                DateTime startEntry = endEntry.AddMinutes(-currentLSIP);
                            ...
                            else
                            {
                                DateTime DateTimeToSet = meterRTC;   //.AddMinutes(currentLSIP);
                                NextIP(ref meterRTC, currentLSIP, 59, 1);
                                WrapperObj.SetObjectValue(8, "0.0.1.0.0.255", 2, SetGetFromMeter.GetRTCSetString(DateTimeToSet));
                                SetGetFromMeter.Wait(2 * 1000);
                                SetGetFromMeter.GetDataFromWrapperObject(ref WrapperObj, (ObjectType)8, "0.0.1.0.0.255", 2, out meterRTCData);
                                meterRTC = DateTime.ParseExact(parse.GetProfileValueString(meterRTCData.Trim()).Trim(), "dd/MM/yyyy hh:mm:ss tt", CultureInfo.InvariantCulture);
                                DateTime endEntry = new DateTime(meterRTC.Year, meterRTC.Month, meterRTC.Day, meterRTC.Hour, meterRTC.Minute, 0);
                            ...
                            var blockloadData = DLMSProfileGenericHelper.GetProfileDataTable(ref WrapperObj, "1.0.99.1.0.255", "1.0.94.91.4.255", 0, 0, (byte)1, startDT, endDT);
                            blockloadData = WrapperObj.GetSingleEntryLSorDEDataTable(blockloadData);
                            //var blockloadData = DLMSObj.GetSingleEntryLSorDEDataTable("1.0.99.1.0.255", "1.0.94.91.4.255", startDT, endDT, 1);
                            blockloadDT = ((DataTable)blockloadData["DataTable"]).Copy();
                            IsLSAvailable = true;
                            break;
                        ...
                        case "1.0.99.2.0.255":
                            startDT = string.Empty;
                            endDT = string.Empty;
                            SetGetFromMeter.GetDataFromWrapperObject(ref WrapperObj, (ObjectType)7, "1.0.99.2.0.255", 7, out inUseEntriesData);
                            inUseEntries = Convert.ToInt32(parse.GetProfileValueString(inUseEntriesData));
                            SetGetFromMeter.GetDataFromWrapperObject(ref WrapperObj, (ObjectType)8, "0.0.1.0.0.255", 2, out meterRTCData);
                            meterRTC = DateTime.ParseExact(parse.GetProfileValueString(meterRTCData).Trim(), "dd/MM/yyyy hh:mm:ss tt", CultureInfo.InvariantCulture);
                            if (inUseEntries >= 1)
                            {
                                DateTime startEntry = new DateTime(meterRTC.Year, meterRTC.Month, meterRTC.Day, 00, 00, 00);
                                DateTime endEntry = new DateTime(meterRTC.Year, meterRTC.Month, meterRTC.Day, 23, 00, 00);
                            ...
                            var dailyloadData = DLMSProfileGenericHelper.GetProfileDataTable(ref WrapperObj, "1.0.99.2.0.255", "1.0.94.91.5.255", 0, 0, (byte)1, startDT, endDT);
                            dailyloadData = WrapperObj.GetSingleEntryLSorDEDataTable(dailyloadData);
                            //var dailyloadData = DLMSObj.GetSingleEntryLSorDEDataTable("1.0.99.2.0.255", "1.0.94.91.5.255", startDT, endDT, 1);
                            dailyloadDT = ((DataTable)dailyloadData["DataTable"]).Copy();
                            IsDEAvailable = true;
                            break;
                        //Voltage Related Events Profile
                        case "0.0.99.98.0.255":
                            var voltageData = WrapperObj.GetSingleEntryProfileDataTable($"{selectedProfile.Split('-')[1].Trim()}", $"{selectedProfile.Split('-')[2].Trim()}");
                            voltageDT = ((DataTable)voltageData["DataTable"]).Copy();
                            IsVoltageAvailable = true;
                            break;
                        //Current Related Events Profile
                        case "0.0.99.98.1.255":
                            var currentData = WrapperObj.GetSingleEntryProfileDataTable($"{selectedProfile.Split('-')[1].Trim()}", $"{selectedProfile.Split('-')[2].Trim()}");
                            currentDT = ((DataTable)currentData["DataTable"]).Copy();
                            IsCurrentAvailable = true;
                            break;
                        //Power Related Events Profile
                        case "0.0.99.98.2.255":
                            var powerData = WrapperObj.GetSingleEntryProfileDataTable($"{selectedProfile.Split('-')[1].Trim()}", $"{selectedProfile.Split('-')[2].Trim()}");
                            powerDT = ((DataTable)powerData["DataTable"]).Copy();
                            IsPowerAvailable = true;
                            break;
                        //Transaction Events Profile
                        case "0.0.99.98.3.255":
                            var transactionData = WrapperObj.GetSingleEntryProfileDataTable($"{selectedProfile.Split('-')[1].Trim()}", $"{selectedProfile.Split('-')[2].Trim()}");
                            transactionDT = ((DataTable)transactionData["DataTable"]).Copy();
                            IsTransactionsAvailable = true;
                            break;
                        //Other Tamper Events Profile
                        case "0.0.99.98.4.255":
                            var otherData = WrapperObj.GetSingleEntryProfileDataTable($"{selectedProfile.Split('-')[1].Trim()}", $"{selectedProfile.Split('-')[2].Trim()}");
                            otherDT = ((DataTable)otherData["DataTable"]).Copy();
                            IsOtherAvailable = true;
                            break;
                        //Non roll over Events Profile
                        case "0.0.99.98.5.255":
                            var nonrolloverData = WrapperObj.GetSingleEntryProfileDataTable($"{selectedProfile.Split('-')[1].Trim()}", $"{selectedProfile.Split('-')[2].Trim()}");
                            nonrolloverDT = ((DataTable)nonrolloverData["DataTable"]).Copy();
                            IsNonRolloverAvailable = true;
                            break;
                        //Control Events Profile
                        case "0.0.99.98.6.255":
                            var controlData = WrapperObj.GetSingleEntryProfileDataTable($"{selectedProfile.Split('-')[1].Trim()}", $"{selectedProfile.Split('-')[2].Trim()}");
                            controlDT = ((DataTable)controlData["DataTable"]).Copy();
                            IsControlAvailable = true;
                            break;
                    }
                ...
                for (int profileIndex = 0; profileIndex < availableProfileList.Count; profileIndex++)
                {
                    ...
                    foreach (DataRow selectedRow in printeingDT.Rows)
                    {
                        ...
                        {
                            SetGetFromMeter.GetDataFromWrapperObject(ref WrapperObj, (ObjectType)Convert.ToInt32(interfaceClass), obis, 3, out scalerData);
                            scaler = ProfileSheetConfiguration.standardScalerList.FirstOrDefault(s => s.Contains($"{scalerData.Substring(6, 2)}"));
                            unit = ProfileSheetConfiguration.standardUnitList.FirstOrDefault(s => s.Contains($"{scalerData.Substring(10, 2)}"));
                        }
                        else
                        ...
                        {
                            if (SetGetFromMeter.GetDataFromWrapperObject(ref WrapperObj, (ObjectType)Convert.ToInt32(interfaceClass), obis, Convert.ToInt32(attribute), out dataType))
                            {
