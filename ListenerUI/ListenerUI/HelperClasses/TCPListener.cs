...
namespace MeterReader.DLMSNetSerialCommunication
{
    ...
    {
        public static DataTable ListnerDataTable { get; set; } = new DataTable()
        {
            Columns =
            {
                new DataColumn("Time Stamp", typeof(string)),
                new DataColumn("Ciphered Data", typeof(string)),
                new DataColumn("Without Ciphered", typeof(string)),
        ...
        public TCPListener(string hostName, int port)
        {
            // TODO: Must set communication specific settings.
            media = new GXNet(NetworkType.Tcp, hostName, port);
                    ...
                    if (notify.IsComplete)
                    {
                        ...
                        if (!notify.IsMoreData)
                        {
                            try
                            {
                                ...
                                //Comment this if the meter describes the content of the push message for the client in the received data.
                                //foreach (GXKeyValuePair<GXDLMSObject, GXDLMSCaptureObject> it in clone.PushObjectList)
                                //{
                            ...
                            try
                            {
                                ...
                                if (notify.Value is List<object>)
                                {
                                    ...
                                    List<GXKeyValuePair<GXDLMSObject, int>> objects = client.ParsePushObjects((List<object>)tmp[0]);
                                    //Remove first item because it's not needed anymore.
                                    objects.RemoveAt(0);
                                    //Update clock.
                                    int Valueindex = 1;
                                    foreach (GXKeyValuePair<GXDLMSObject, int> it in objects)
                                    {
